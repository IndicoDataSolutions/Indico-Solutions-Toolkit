...
def test_grouper_row_number_false_add_to_all(
    ...
    assert len(three_row_invoice_preds) - 1 == len(
        litems._line_item_predictions
    ), "all predictions present in update"
    assert (
        len(litems._non_line_item_predictions) == 1
    ), "non-line item field not ignored"
    assert len(three_row_invoice_preds) == len(
        litems.updated_predictions.to_list()
    ), "should include all original preds"
    ...
    for pred in litems._line_item_predictions:
        if "row 1" in pred["text"]:
            ...
            row_count[pred["row_number"]].append(pred["label"])
    assert 1 in row_count and set(row_count[1]) == set(
        ["line_date", "work_order_tonnage"]
    )
    assert 2 in row_count and set(row_count[2]) == set(["work_order_number"])
    assert 3 in row_count and set(row_count[3]) == set(
        ["work_order_number", "line_date"]
    )
...
def test_grouper_no_token_match_no_exception(
    ...
    assert (
        litems._errored_predictions[0]["error"]
        == "No matching token found for line item field"
    )
