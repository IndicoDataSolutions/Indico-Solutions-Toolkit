...
def test_create_classifier_missing_files(indico_client, testdir_file_path):
    aclass = AutoClassifier(
        indico_client, os.path.join(testdir_file_path, "data/auto_class")
    )
    aclass.file_texts = [None, "a"]
    aclass.file_classes = ["c", "d"]
    aclass.file_paths = ["something", "something1"]
    with tempfile.NamedTemporaryFile(suffix=".csv") as tf:
        aclass.to_csv(tf.name)
        df = pd.read_csv(tf.name)
        assert df.shape[0] == 1
        assert df.shape[1] == 3
        assert "a" in df["text"].tolist()
