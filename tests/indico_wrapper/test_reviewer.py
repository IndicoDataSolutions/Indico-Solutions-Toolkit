...
def submissions_awaiting_review(workflow_id, workflow_wrapper, pdf_filepath):
    """
...
def get_change_formatted_predictions(workflow_result):
    ...
    """
    return {workflow_result.model_name: workflow_result.pre_review_predictions}
...
def test_reject_from_review(submissions_awaiting_review, reviewer_wrapper):
    id_in_review = reviewer_wrapper.get_random_review_id()
...
def test_reject_from_admin_review(submissions_awaiting_review, reviewer_wrapper):
    id_in_exception = reviewer_wrapper.get_random_exception_id()
    submission = reviewer_wrapper.indico_client.call(GetSubmission(id_in_exception))
    assert submission.status == "PENDING_ADMIN_REVIEW"
