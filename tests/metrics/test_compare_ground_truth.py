...
@pytest.mark.parametrize("input, expected", ([ground_truth, predictions], expected))
def compare_ground_truth(input, expected):
    cgt_instance = CompareGroundTruth(input)
    are_metrics_correct = cgt_instance._build_metrics_dicts(input)
