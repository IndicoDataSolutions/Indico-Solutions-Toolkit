...
# TODO: test toc_page and different colored highlights
...
def invoice_ocr_obj(highlighter_data_path):
    return pickle.load(
        open(os.path.join(highlighter_data_path, "ocr_obj.p"), "rb")
    )

@pytest.fixture(scope="session")
def invoice_predictions(highlighter_data_path):
    return pickle.load(
        open(os.path.join(highlighter_data_path, "predictions.p"), "rb")
    )


def test_highlighter_collect_tokens(invoice_predictions, invoice_ocr_obj, highlighter_pdf_path):
...
def test_highlighter_highlight_pdf(invoice_predictions, invoice_ocr_obj, highlighter_pdf_path):
    highlight = Highlighter(invoice_predictions, highlighter_pdf_path)
    highlight.collect_tokens(invoice_ocr_obj.token_objects)
...
def test__get_page_label_counts(invoice_predictions, invoice_ocr_obj, highlighter_pdf_path):
    highlight = Highlighter(invoice_predictions, highlighter_pdf_path)
    highlight.collect_tokens(invoice_ocr_obj.token_objects)
